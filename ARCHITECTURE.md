...
### Tests (run via `npm test` — **870 tests across 89 files** as of 2026-08-14; the list in `package.json` is explicit — add new test files there. The ship gate re-measures this header's pair whenever a battery run exists, so a stale edit is refused at merge)
