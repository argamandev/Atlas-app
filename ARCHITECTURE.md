...
| `apiAuthBoundary.test.ts` | Build-enforced guard: every exported HTTP handler under `src/app/api` must resolve a signed-in user, or be listed in its `PUBLIC` allowlist **with a reason**. Also bans the `DEMO_USER_ID` fallback. Added 2026-08-03 because the holes it closes were months of drift, not one mistake — and because the fleet's own notes described them as "two routes" when a command found 16 sites in 8 files. It is a TEXT scan: it proves each method calls an auth helper, not that the caller may touch the row it reads (that is the `lib/db` modules' job — most still use `supabaseAdmin`, which bypasses RLS). |
