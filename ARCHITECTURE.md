...
### Tests (run via `npm test` — **1153 tests across 107 files** as of 2026-08-15; the list in `package.json` is explicit — add new test files there. The ship gate re-measures this header's pair whenever a battery run exists, so a stale edit is refused at merge)
...
· `workspace/intake/companyPin.test.ts` · `workspace/intake/findSources.test.ts` · `workspace/intake/json.test.ts`
