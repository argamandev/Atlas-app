...
| `db/compositeChildFk.test.ts` | Build-enforced guard, `rules/db.md`'s ownership law one level deeper: text-scans `supabase/migrations/*.sql` for a table with an owner RLS policy (`using (auth.uid() = user_id)`, matched in both the per-table and 016's loop-generated shape) and fails for any child FK into it whose REFERENCING column list does not contain `user_id` — PostgreSQL referential-integrity checks bypass RLS, so a plain `agent_id → agents(id)` validates a row pointed at a stranger's parent row. Added after migration 032's first draft shipped exactly that shape across four new tables despite the spec already naming 015/016 as the pattern to copy. Round 6 replaced the original decision — FK ARITY (`columnCount > 1`), a PROXY for the property (`rules/app.md` M3 clause 2) that passed `foreign key (agent_id, created_at) references public.agents (id, created_at)` green, two columns and zero ownership binding — with the fact itself, and restated the law in `rules/db.md` to match (it had said "never single-column", i.e. the same proxy, so declaration and mechanism agreed with each other and both missed the point); the same round taught the table-level branch the unqualified `references agents (…)` form, which until then fell through to the inline regex and misreported a CORRECT composite FK as single-column while the stated limits claimed both forms were handled. Both directions verified by probe against a copied migration tree, never by reading. One grandfathered pre-existing violation (`quotes.folder_id → quote_folders`, migration 010, predates the law) is allowlisted by exact (file, table) pair. Comments are blanked with CRLF normalised first (round 5: the unstripped version was a no-op on every CRLF migration in the repo, proven both directions — an injected reference inside an existing comment false-flagged a correct file, and a commented-out policy red-lined `companies`' seven legitimate refs — closed with a canary that fails if blanking a known-CRLF fixture ever stops working). States its own limits: a text scan, not a catalog walk; three policy shapes it has never had to match (the comparison reversed, wrapped in a sub-select — Supabase's own recommended performance form — or keyed on a column other than `user_id`) would misdetect a genuinely owner-scoped table as unscoped, and pre-existing, not caused by this test: `watchlist`/`notification_prefs`/`sent_alerts` use a quoted policy name over an unqualified (non-`public.`-prefixed) table and are consequently never detected as owner-scoped by either the old or the new regex. Two further stated limits from round 6: an INLINE constraint is treated as never keyed through `user_id` (a single referencing column cannot carry both the child id and `user_id`), which over-reports the one shape nothing here writes — a deliberate inline `user_id … references public.<owner-scoped>(user_id)`; and the referencing column list is read textually, so a `user_id` written as anything but a plain or double-quoted `user_id` token is not recognised. |
