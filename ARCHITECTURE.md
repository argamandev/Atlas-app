...
### Tests (run via `npm test` — **1090 tests across 106 files** as of 2026-08-15; the list in `package.json` is explicit — add new test files there. The ship gate re-measures this header's pair whenever a battery run exists, so a stale edit is refused at merge)
...
· `chat/projectContext.test.ts` · `chat/turnRoute.test.ts`
