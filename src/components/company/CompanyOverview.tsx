  ...
  return (
    <div className="space-y-7">
      ...
      <section className="flex animate-fade-up flex-col gap-3.5" style={{ animationDelay: '0.12s' }}>
        ...
        <div
          className="overflow-hidden rounded-[16px] border border-[#E6E2DA] bg-[#FCFBF8]"
          style={{ boxShadow: '0 1px 3px rgba(28,27,25,.045), 0 14px 34px -26px rgba(28,27,25,.14)' }}
        >
          <div className="flex items-center justify-between gap-3.5 border-b border-[#ECE7DD] px-[22px] py-[17px]">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0 text-start">
                ...
                <div className="mt-0.5 font-mono-num text-[11.5px] text-[#8A867C]" dir="ltr">
                  {dict.company.reportedOn} {formatDate(stub.reported.reportedAtIso, locale)} ·{' '}
                  {dict.company.readFromCall}
                </div>
              </div>
            </div>
            <span className="inline-flex flex-none items-center gap-[7px] rounded-full border border-[#E4DED1] bg-[#F2EEE6] px-3 py-1.5 text-[12.5px] text-ink">
              <span>▴</span>
          ...
          >
            <div className="mb-3 flex items-center gap-[9px] font-mono-num text-[10px] uppercase tracking-[0.18em] text-[#8A867C]">
              {dict.company.fromTheCall}
              <span className="h-px flex-1 bg-[#E4DED1]" />
            </div>
            <p
              dir="rtl"
            ...
            <div className="mt-3.5 flex items-center gap-3.5">
              <span className="font-mono-num text-[11px] uppercase tracking-[0.05em] text-[#8A867C]">
                {dict.company.speakerLabel} · {stub.reported.speakerRole}
              </span>
              {latest && (
                <Link
                  href={`/app/live/${latest.id}`}
                  className="hov-borderi inline-flex items-center gap-1.5 border-b border-[#E4DED1] font-mono-num text-[11.5px] text-ink"
                >
          ...
          <div className="flex items-center gap-2.5 border-t border-[#ECE7DD] px-[22px] py-[13px]">
            {latest ? (
              <Link
                href={`/app/live/${latest.id}`}
                className="hov-doc inline-flex items-center gap-2 rounded-[9px] border border-[#E6E2DA] px-[13px] py-2 text-[13px] text-ink"
              >
                <TranscriptIcon size={14} className="text-[#8A867C]" />
                {dict.company.transcript}
              </Link>
            ) : (
              <span className="inline-flex items-center gap-2 rounded-[9px] border border-dashed border-[#E6E2DA] px-[13px] py-2 text-[13px] text-ink-faint">
            ...
            <Link
              ...
              className="hov-ink ms-auto inline-flex items-center gap-[7px] text-[13px] text-[#8A867C]"
            >
              <SparkleIcon size={20} />
        ...
        <div className="overflow-hidden rounded-card border border-[#E6E2DA] bg-paper">
          {stub.announcements.map((a) => (
            <button
              ...
              type="button"
              className="hov-fill flex w-full items-center gap-[13px] border-b border-[#ECE7DD] px-4 py-3 text-start last:border-b-0"
            >
              <span className="w-[72px] flex-none font-mono-num text-[11px] text-[#9A968C]" dir="ltr">
                {formatDate(a.dateIso, locale)}
              </span>
              <span className="flex-none whitespace-nowrap rounded-[5px] border border-[#E0DACE] px-2 py-[2px] text-center text-[10px] font-semibold uppercase tracking-[0.06em] text-[#8A867C]">
                {tagLabel[a.tag]}
              </span>
              ...
              <ChevronRightIcon
                size={15}
                strokeWidth={1.7}
                className="flex-none text-[#C7C2B6] rtl:rotate-180"
              />
            </button>
          ))}
        </div>
      ...
      {peers.length > 0 && (
        <section className="animate-fade-up" style={{ animationDelay: '0.18s' }}>
          <div className="mb-3.5 flex items-baseline gap-2.5">
            ...
            <span className="text-[12px] text-[#A8A498]">{dict.company.relatedSub}</span>
          </div>
            ...
            {peers.map((p) => {
              ...
              return (
                <Link
                  ...
                  className="hov-peer flex items-center gap-3 rounded-[11px] border border-[#E6E2DA] bg-[#FCFBF8] px-[15px] py-3.5 text-start"
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-0.5 truncate font-mono-num text-[11.5px] text-[#8A867C]" dir="ltr">
