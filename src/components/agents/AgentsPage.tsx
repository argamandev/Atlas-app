...
export function AgentsPage({
  ...
  // Widened, the agent panel owns the whole main area — the deck and the grid
  // step aside rather than being squeezed. The demo banner stays: the design has
  // no notion of demo content, and marking it is not optional here.
  const wide = !!dockAgent && !!dock?.wide

  return (
    <div className="flex h-full min-h-0 flex-col">
      <DemoBanner />

      {/* The panel is a SIBLING of the whole left column, not of the scroll area
          below the deck — so it runs the full height and the black command deck
          stops where the panel starts, as the design draws it. */}
      <div className="flex min-h-0 flex-1">
              ...
              <div className="mx-auto w-full max-w-[1160px] px-10 pb-32 pt-9">
                <div className="flex flex-wrap items-start justify-center gap-11">
                          ...
                          {menuFor === a.id && (
                            <div className="absolute inset-x-2.5 top-[calc(100%-6px)] z-30 rounded-[11px] border border-hairline bg-canvas p-[5px] shadow-menu">
                              <button
                                type="button"
                                onClick={() => {
                                  setDock({ id: a.id, tab: 'profile', wide: false })
                                  setMenuFor(null)
                                }}
                              ...
                              {a.findings.length > 0 ? (
                                <button
                                  type="button"
                                  onClick={() => {
                                    setDock({ id: a.id, tab: 'chat', wide: false })
                                    setMenuFor(null)
                                  }}
                                  className="flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2.5 text-start hover:bg-subtle"
                                >
                    ...
                    <div className="flex flex-col overflow-hidden rounded-[10px] border border-hairline bg-paper">
                      {finished.map((f, i) => (
                        <button
                          key={f.id}
                          type="button"
                          onClick={() => setDock({ id: f.agentId, tab: 'chat', wide: false })}
                  ...
                  <div className="min-w-[280px] max-w-[340px] flex-[1_1_300px]">
                    ...
                    {/* Recent agent chats — the way back into a conversation you
                    already had with an agent. Opening one replays that question. */}
                    <div className={`${sectionLabel} mb-3 mt-[30px]`}>{dict.agents.recentAgentChats}</div>
                    <div className="flex flex-col gap-px">
                      {recent.map((r) => (
                        <button
                          ...
                          type="button"
                          onClick={() =>
                            setDock({ id: r.agentId, tab: 'chat', wide: false, seed: r.question })
                          }
                          className="flex items-start gap-2.5 rounded-lg px-2.5 py-2 text-start transition-colors hover:bg-subtle/60"
                        >
                          <span className="mt-[3px] flex-none text-ink">
                            <SparkleIcon size={13} />
                          </span>
                          <span className="flex min-w-0 flex-1 flex-col gap-px">
                            <span dir="auto" className="truncate text-[13px] text-ink">
                              {r.question}
                            </span>
                            <span
                              dir="ltr"
                              className="truncate font-mono-num text-[11px] text-ink-ghost rtl:text-right"
                            >
                              {r.agentName} · {r.when}
                            </span>
                          </span>
        ...
        {dockAgent && (
          <AgentDock
            ...
            wide={dock!.wide}
            onWide={(w) => setDock((d) => (d ? { ...d, wide: w } : d))}
            seedQuestion={dock!.seed ?? null}
          />
        )}
      </div>
      ...
      {creating && (
        <CreateAgent
          ...
          onCreate={(a) => {
            const id = addAgent(a)
            setCreating(false)
            setDock({ id, tab: 'profile', wide: false })
          }}
        />
      )}
    </div>
