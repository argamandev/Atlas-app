...
export default function EntryGate() {
  ...
  return (
    <div className="relative bg-bg min-h-screen overflow-x-hidden" dir="rtl">
      ...
      {/* Top bar */}
      <header
        className="fixed top-0 right-0 left-0 z-20 flex items-center justify-between px-8 py-4 border-b border-border/40 bg-bg/80 backdrop-blur-sm"
        dir="ltr"
      >
        <div className="flex items-center gap-2">
          <span className="w-1.5 h-1.5 rounded-full bg-success animate-blink" />
          <span className="font-mono-num text-2xs text-muted tracking-widest uppercase">
            ...
            <div className="flex flex-col gap-5 text-right max-w-sm mx-auto lg:mx-0">
              {CAPABILITIES.map((c) => (
                <div key={c.num} className="flex items-start gap-4">
                  <span className={`font-mono-num text-2xs tracking-widest mt-1 flex-shrink-0 ${c.color}`}>
                    {c.status}
                  </span>
      ...
      {/* Footer */}
      <footer
        className="relative z-10 border-t border-border/40 px-8 py-4 flex items-center justify-between"
        dir="ltr"
      >
        <span className="font-mono-num text-2xs text-muted tracking-widest">
          © 2026 SENTIMENT INTELLIGENCE LTD.
        </span>
