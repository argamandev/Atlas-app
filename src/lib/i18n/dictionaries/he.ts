...
export const he: Dictionary = {
  ...
  workspace: {
    ...
    // הבקשה הראשונה של המייסד ב-15.8: "תגיד לי שאתה לא מבין את השם הזה, ותבקש
    // את השם הרשמי". עכשיו יש תשובה מדויקת יותר מ"תכתוב אחרת" — לבחור עם @,
    // ואז השם לא נכתב בכלל אלא נבחר.
    intakeUnknownCompany: 'לא זיהיתי חברה בשם הזה. אפשר לבחור אותה מהרשימה עם @ — ככה זה תמיד מדויק.',
    // לא השורה שמעל: את זה רואה מי שכבר בחר עם @, ולהציע לו @ זה להציע בדיוק
    // את מה שהרגע לא עבד.
    intakePinnedCompanyUnreachable:
      'לחברה הזו אין מנפיק בבורסה שאפשר לחפש לפיו דיווחים, אז זה מכסה רק את מה שכבר נמצא באטלס.',
    // בחירת חברה ב-@ — השם לא נכתב, הוא נבחר, ולכן אי אפשר לטעות בו.
    intakeMention: 'לבחור חברה',
    intakeUnpinCompany: 'להסיר את החברה',
    ...
    intakeRequestPartlyUnderstood:
      'את החברה זיהיתי, אבל לא הצלחתי להבין לאיזו תקופה או לאיזה סוג מסמך התכוונת — אז אלה הדיווחים האחרונים שלה. אפשר להגיד שנה או סוג דוח ואצמצם.',
  ...
  chat: {
    ...
    // כרטיס 08b, הזרקת שיחה שלמה. השיחה לא נכנסה כולה לתור אחד, ולכן המודל קרא
    // רק את תחילתה. נאמר על התשובה עצמה — תשובה שנשענת על חלק מהשיחה אסור שתיראה
    // כמו תשובה שנשענת על כולה.
    callTruncated: 'השיחה ארוכה מכדי להיקרא במלואה — התשובה מבוססת על תחילתה בלבד.',
    // כרטיס 08c-2, כתוביות חי. אותה נסיגה — אבל החצי ההפוך, ולכן זו אינה המחרוזת
    // שמעליה: שיחה שמורה נקראת מתחילתה, ושיחה חיה נקראת מהכתוביות האחרונות, כי
    // עליהן הצופה שואל. לומר לצופה בשידור חי שהתשובה מבוססת על "תחילת השיחה"
    // היה מצביע דווקא על הקטע שהמודל לא ראה.
    liveTruncated: 'השיחה מתארכת — התשובה מבוססת על החלק האחרון שלה, לא על כל מה שנאמר עד כה.',
    // כרטיס 08c-3, הדוח המצורף. שתי מחרוזות ולא אחת, כי אלו שתי עובדות שונות:
    // חלק מהקטע שסומן הגיע לתשובה, לעומת טקסט דוח שלא הגיע כלל. משפט משותף היה
    // גורם לשנייה להיקרא כמו הראשונה.
    //
    // ושתיהן נייטרליות לגבי הסיבה — ובתחילה לא היו. הפסקה שעמדה כאן אמרה שקטע
    // שנחתך פירושו "סמן קטע קצר יותר". הקוד לעולם אינו קובע זאת: המצב הזה מוחזר
    // גם כשעמוד מסומן אחד לא היה קריא או לא נמצא לו טקסט שמור, ולכן שיוך לאורך
    // שולח את המשתמש לסמן קטע קצר יותר שלא ישנה דבר. מה שנכון תמיד, והוא גם מה
    // שאפשר לפעול לפיו, הוא כמה ממה שסומן נמצא בתשובה.
    reportTruncated: 'רק חלק מהקטע שסומן בדוח הגיע לתשובה הזו.',
    reportFailed: 'לא ניתן היה לטעון את טקסט הדוח, ולכן התשובה נכתבה בלעדיו.',
    pageShort: 'עמ׳',
    // ─── כרטיס 07: מצב ההיצמדות של המנוע החדש, אמור בגלוי ────────────────────
    // מצב חיפוש גלוי בכוונה (מפרט §2.3). הוא נקבע מתוך ההיקף בלבד — האם זוהתה
    // חברה — ולעולם לא מנוחש מתוך השאלה, כך שכשהוא שגוי המשתמש רואה זאת ומתקן
    // בלחיצה אחת.
    searchMode: 'מצב חיפוש',
    searchModeHint: 'לא זוהתה חברה, ולכן אטלס עונה עם ממצאים מכל השוק.',
    pinCompany: 'הצמדה לחברה',
    unpinCompany: 'חיפוש בכל השוק במקום',
    // מוצג כשחברה זוהתה בשרת אך שמה לא נטען. ההצמדה אמיתית, והמשתמש חייב לראות
    // אותה ולהיות מסוגל לבטלה.
    pinnedUnknownCompany: 'חברה',
    // ─── תשע הסיבות לכך שתשובה אינה שלמה ─────────────────────────────────────
    // אחת לכל `ClientIncompleteCode`. השרת שולח קוד בדיוק כדי שהמסך הזה לא
    // יצטרך לנתח את נוסח השגיאה האנגלי כדי להחליט מה להציג בעברית.
    incomplete: {
      roundTripCap: 'אטלס הגיע למגבלת שלבי החיפוש לפני שסיים — נסו שאלה ממוקדת יותר.',
      allSourcesFailed: 'כל חיפושי המקורות בתשובה הזו נכשלו, ולכן דבר בה אינו מבוסס — אל תסתמכו עליה.',
      unverifiedQuote: 'ציטוט בתשובה הזו לא אומת מול המקור שלו.',
      lengthLimit: 'התשובה הגיעה למגבלת האורך לפני שהסתיימה.',
      modelRefused: 'המודל בחר שלא להמשיך את התשובה הזו.',
      modelPaused: 'המודל השהה את התשובה לפני שסיים אותה.',
      stoppedUnknown: 'התשובה נעצרה באופן בלתי צפוי לפני שהסתיימה.',
      noAnswerText: 'לא התקבל טקסט תשובה.',
      streamEnded: 'החיבור נותק לפני שהתשובה הסתיימה.',
    },
    suggestions: [
  ...
  // מנהל · מצב האינדוקס של הקורפוס (פרוסה A5).
  corpusAdmin: {
    title: 'אינדקס הקורפוס',
    subtitle: 'מה הקורפוס המשותף מחזיק, ומה טרם סיים להצטרף אליו.',
    adminOnly: 'הדף הזה מיועד למנהלי המערכת.',
    loadFailed: 'לא ניתן היה לקרוא את מצב האינדוקס של הקורפוס.',
    transcripts: 'תמלולים',
    documents: 'דוחות',
    chunks: 'מקטעים לחיפוש',
    structuredFacts: 'נתונים מובְנים (XBRL)',
    indexed: 'מאונדקס',
    pending: 'ממתין',
    failed: 'נכשל',
    excluded: 'מוחרג',
    other: 'מצב לא מזוהה',
    factsYes: 'נותחו',
    factsNone: 'אין נתונים מובְנים',
    factsFailed: 'הניתוח נכשל',
    factsUnknown: 'טרם נבדק',
    settled: 'כל המקורות סיימו להתאנדקס.',
    troubledTitle: 'טרם סיימו להתאנדקס',
    troubledEmpty: 'אין מקורות ממתינים או שנכשלו.',
    troubledCapped: 'מוצגים {shown} מתוך {total}. הרשימה מוגבלת; המספר למעלה הוא המלא.',
    excludedNote: 'מקורות מוחרגים הם החלטה מכוונת — תוכן הדגמה וכפילויות ידועות לא נכנסים לקורפוס.',
  },
