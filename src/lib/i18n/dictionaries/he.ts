...
export const he: Dictionary = {
  ...
  chat: {
    ...
    // שרק על אחת מהן אפשר לפעול: קטע שנחתך אומר "סמן קטע קצר יותר", וקטע
    // שנכשל אומר "טקסט הדוח לא הגיע בכלל". משפט משותף היה גורם לשנייה להיקרא
    // כמו הראשונה ושולח את המשתמש לסמן מחדש עמודים שממילא לא נקראו.
    // נייטרלי לגבי הסיבה, ובתחילה לא היה. נכתב "ארוך מכדי להיקרא במלואו" — סיבה
    // שהקוד לעולם אינו קובע: המצב הזה מוחזר גם כשעמוד מסומן אחד לא היה קריא או
    // לא נמצא לו טקסט שמור, ולכן שיוך לאורך שולח את המשתמש לסמן קטע קצר יותר
    // שלא ישנה דבר. מה שנכון תמיד — והוא גם מה שאפשר לפעול לפיו — הוא שהתשובה
    // מכסה רק חלק ממה שסומן.
