...
RECURRENCE: no
Accepted and stated rather than closed: nothing fabricates a chip, but the user
...
RECURRENCE: no
Ordering by which query found a row is a proxy for relevance (M3.2); replaced
...
finished answer.
RECURRENCE: no
History now labels it. (Round 2 found this fix incomplete — see below.)
...
RECURRENCE: no
Stated as a limit of the evidence rather than claimed verified.
