...
- [ ] Review asks explicitly whether a finding is a recurrence of an existing law, and the answer is recorded rather than assumed
- [ ] A recurrence requires a mechanism one tier stronger, shipped in the same commit as the fix
- [ ] The escape hatch exists: a law may be marked unenforceable with a stated reason, and that satisfies the gate
- [ ] Laws that cannot be mechanised appear as fixed checklist items in the ship ritual, not as prose
- [ ] The count of unenforced laws is reported at ship time, so the trend is visible rather than recalled
