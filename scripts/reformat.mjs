...
// Skips download + IVRIT entirely (Gemini → GPT-4.1 fallback organize, then save).
...
console.log(`[reformat] ${id} — status=${data.status}, ${data.raw_transcript.length} chars, word_segments=${data.word_segments ? 'yes' : 'no'} (engine=${engine})`)
...
const { error: updErr } = await supabaseAdmin
  .from('transcripts')
  .update({ formatted_data: formatted, status: 'completed', processing_step: 'completed', error_message: null })
  .eq('id', id)
if (updErr) {
...
console.log(`[reformat] DONE — ${id} completed in ${((Date.now() - t0) / 1000).toFixed(1)}s (${speakers} speakers, ${lines} lines)`)
process.exit(0)
